...
- **Run pre-commit hooks** - Always run `pre-commit run --all-files` before committing. This includes **`verify-python-project-standards`** ([`scripts/verify-standards.sh`](scripts/verify-standards.sh) plus optional [`scripts/check_lint_baseline.py`](scripts/check_lint_baseline.py): layout, ruff/mypy in config, CI references org reusables, **`STANDARDS_VERSION`** vs **`@v…`** pins; strict baseline check skipped via hook env until adoption), linting, formatting, type checking, assert statement checks, debug statement detection, and other quality checks. Pre-commit hooks are automatically enforced, but running them manually helps catch issues early.
