...
- **`tapes/`** – VHS tape sources (`.tape` files). **`sample.mp3`**, **`sample.flac`** – Demo audio (tracked). **`sample.wav`** – Created when building (gitignored). Temp copies used by tapes (`demo_vorbis_before.flac`, `demo_vorbis_after.flac`, `demo_id3v1_before.mp3`, `demo_id3v1_after.mp3`, `demo_write.mp3`) are gitignored.
- **`output/`** – Build output (gitignored): tape GIFs, hero video, side-by-side GIFs, page segments. All scripts write here.
- **Hero video:** `output/one_to_rule_them_all.mp4` (intro + 4 pages, 2 panels each). All cell GIFs come from tapes; **no placeholders**. Script fails early if logo or any required tape output is missing. Scenario: [VIDEO_SCENARIO_ONE_TO_RULE.md](VIDEO_SCENARIO_ONE_TO_RULE.md).
...
- **Prerequisites:** [VHS](https://github.com/charmbracelet/vhs), `ttyd`, `ffmpeg`, `ffprobe`, `metaflac`, `mid3v2`. macOS: `brew install vhs`. Logo: `assets/logo.png` or `one_to_rule/logo.png`. Hero video requires all tapes to run successfully (no placeholders).
...
- **Hero video:** `./generate_one_to_rule_video.sh` (from this dir; run from repo root so paths resolve) → `output/one_to_rule_them_all.mp4`.
