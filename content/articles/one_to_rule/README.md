...
Hero video + linked-post assets. Build output goes under `output/` (`work/` + `final/`, gitignored).
...
- **`tapes/`** – VHS sources. **`Output`** in each tape is `output/work/<name>.gif`; run **`vhs` with cwd = this article dir** (see `scripts/generate_one_to_rule_video.sh`, `scripts/run_demo_side_by_side*.sh`, `run_demo_tape.py`) so the shell starts here—**do not type `cd` in the tape** (VHS only allows `Set Shell "bash"` etc.). **`output/work/demo_read_id3v1.mp3`**, **`output/work/demo_read_vorbis.flac`** – generated for ID3v1/Vorbis cells (under `output/work/`, gitignored).
...
- **`scripts/ensure_demo_read_id3v1.py`** – builds `output/work/demo_read_id3v1.mp3` from `samples/sample.mp3` (ID3v1-only tag, same song). **`scripts/sync_article_sample_metadata.py`** – aligns ID3/Vorbis tags on `samples/sample.{mp3,flac}`. **`scripts/print_id3v1_tags.py`** – prints TAG fields for the left panel.
...
- **Hero video:** `bash content/articles/one_to_rule/scripts/generate_one_to_rule_video.sh` — add `--skip-gifs` to reuse existing `output/work/*.gif`, or `--final-name <file>.mp4` to control the final filename under `output/final/`.
