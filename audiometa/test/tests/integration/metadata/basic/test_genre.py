...
class TestGenreMetadata:
    ...
    def test_genre_metadata_mp3(self, sample_mp3_file: Path, temp_audio_file: Path):
        ...
        test_metadata = {AppMetadataKey.GENRE: test_genre}
    ...
    def test_genre_metadata_edge_cases(self, sample_mp3_file: Path, temp_audio_file: Path):
        ...
        # Test short genre
        short_genre = "A"
        update_file_metadata(temp_audio_file, {AppMetadataKey.GENRE: short_genre})
        metadata = get_merged_app_metadata(temp_audio_file)
        assert metadata.get(AppMetadataKey.GENRE) == short_genre
        
        # Test long genre
        long_genre = "A" * 1000
        update_file_metadata(temp_audio_file, {AppMetadataKey.GENRE: long_genre})
        metadata = get_merged_app_metadata(temp_audio_file)
        assert metadata.get(AppMetadataKey.GENRE) is not None
        assert len(metadata.get(AppMetadataKey.GENRE)) > 0
        
        # Test empty genre
        empty_genre = ""
        update_file_metadata(temp_audio_file, {AppMetadataKey.GENRE: empty_genre})
        metadata = get_merged_app_metadata(temp_audio_file)
        assert metadata.get(AppMetadataKey.GENRE) == empty_genre
