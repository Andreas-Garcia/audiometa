...
class TestHeaderDetectionUtilities:
    ...
    def test_header_detection_for_different_formats(self):        
        ...
        with TempFileWithMetadata({"title": "MP3 Test"}, "mp3") as mp3_file:
            assert helper.has_id3v2_header(mp3_file.path), "MP3 should have ID3v2 header"
            assert not helper.has_vorbis_comments(mp3_file.path), "MP3 should not have Vorbis comments"
            assert not helper.has_riff_info_chunk(mp3_file.path), "MP3 should not have RIFF INFO chunk"
            
            # Test header removal
            result = delete_all_metadata(mp3_file.path)
            assert result is True
            assert not helper.has_id3v2_header(mp3_file.path), "ID3v2 header should be removed"
        
        # Test FLAC format
        with TempFileWithMetadata({"title": "FLAC Test"}, "flac") as flac_file:
            # FLAC might have both ID3v2 and Vorbis comments
            headers = helper.get_metadata_headers_present(flac_file.path)
            ...
            headers_after = helper.get_metadata_headers_present(flac_file.path)
        ...
        with TempFileWithMetadata({"title": "WAV Test"}, "wav") as wav_file:
            ...
            headers = helper.get_metadata_headers_present(wav_file.path)
            ...
            headers_after = helper.get_metadata_headers_present(wav_file.path)
    ...
    def test_header_verification_edge_cases(self):
        ...
        with TempFileWithMetadata({}, "mp3") as test_file:
            ...
            headers = helper.get_metadata_headers_present(test_file.path)
            ...
            headers_after = helper.get_metadata_headers_present(test_file.path)
