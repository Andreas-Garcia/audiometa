...
class TestBase255NonProportionalRatingReading:
    ...
    def test_base_255_non_proportional_0_star_flac(self, test_files_dir: Path):
    ...
    def test_base_255_non_proportional_0_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=0.5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        assert rating is not None
        assert isinstance(rating, (int, float))
    ...
    def test_base_255_non_proportional_1_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=1 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        assert rating is not None
        assert isinstance(rating, (int, float))
    ...
    def test_base_255_non_proportional_1_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=1.5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have 0.5 star metadata instead of 1.5 stars
        assert rating is not None
        assert isinstance(rating, (int, float))
        assert rating == 10
    ...
    def test_base_255_non_proportional_2_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=2 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
    ...
    def test_base_255_non_proportional_2_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=2.5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
        assert rating is None
    ...
    def test_base_255_non_proportional_3_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=3 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        assert rating is not None
        assert isinstance(rating, (int, float))
    ...
    def test_base_255_non_proportional_3_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=3.5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
        assert rating is None
    ...
    def test_base_255_non_proportional_4_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=4 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
        assert rating is None
    ...
    def test_base_255_non_proportional_4_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=4.5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
        assert rating is None
    ...
    def test_base_255_non_proportional_5_star_flac(self, test_files_dir: Path):
        file_path = test_files_dir / "rating_id3v2=5 star.flac"
        metadata = get_merged_unified_metadata(file_path, normalized_rating_max_value=100)
        rating = metadata.get(UnifiedMetadataKey.RATING)
        # This file appears to have no rating metadata
        assert rating is None

    def test_base_255_non_proportional_none_rating_wav(self, test_files_dir: Path):
